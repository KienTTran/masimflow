...
class IntroduceLumefantrineMutantParasiteInfo {
  int location;
  DateTime date;
  double fraction;

  IntroduceLumefantrineMutantParasiteInfo({
  ...
  @override
  String toString() {
    return 'IntroduceLumefantrineMutantParasiteInfo(location: $location, date: $date, fraction: $fraction)';
  }
}

class IntroduceLumefantrineMutantParasites extends Event {
  List<IntroduceLumefantrineMutantParasiteInfo> parasiteInfo;

  IntroduceLumefantrineMutantParasites({
    required String id,
  ...
  @override
  IntroduceLumefantrineMutantParasites copy() {
    final (newID, newControllers) = Utils.getNewFormControllers(id, controllers);
    final newParasiteInfo = parasiteInfo.map((info) => IntroduceLumefantrineMutantParasiteInfo(
      location: info.location,
      date: DateTime.fromMillisecondsSinceEpoch(info.date.millisecondsSinceEpoch),
      fraction: info.fraction,
    )).toList();

    return IntroduceLumefantrineMutantParasites(
      id: newID,
      name: name,
  ...
  void addEntry() {
    ...
    parasiteInfo.add(IntroduceLumefantrineMutantParasiteInfo(location: 0, date: newDate, fraction: 0.01));
  ...
  factory IntroduceLumefantrineMutantParasites.fromYaml(Map<dynamic, dynamic> yaml) {
    final String fID = const Uuid().v4();
    final List infoList = yaml['info'] as List;
    final fParasiteInfo = infoList.map((entry) => IntroduceLumefantrineMutantParasiteInfo(
  ...
  @override
  IntroduceLumefantrineMutantParasitesState createState() => IntroduceLumefantrineMutantParasitesState();
}

class IntroduceLumefantrineMutantParasitesState extends EventState<IntroduceLumefantrineMutantParasites> {
  @override
                ...
                Row(
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: widget.eventForm.EventIntFormField('location_$i', widget.parasiteInfo[i].location),
                    ),
                    SizedBox(
                ...
                Padding(
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      widget.eventForm.EventDoubleFormField('fraction_$i', widget.parasiteInfo[i].fraction),
                    ],
                  ),
                )
              ],
